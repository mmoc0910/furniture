...
const Order = ({ order, state, leadtime }) => {
  ...
  return (
    <div className="flex flex-col gap-3 px-5 pb-8 py-4 rounded-lg bg-[#efefef]">
      ...
      {order.status === 2 && !order?.isRate && (
        <Button
          className="px-2 py-1 rounded-md bg-[#4d7a94] text-white max-w-max ml-auto"
...
const ModalReview = ({ orderId, products, onClick, setShow }) => {
  const [reviews, setReviews] = React.useState([]);
  React.useEffect(() => {
    setReviews(
      [...products].map((product) => ({
        product_id: product.productId,
        product_name: product.productInfo.productName,
        product_image: product.productInfo.images[0],
  ...
  const onMouseEnter = (value, product_id) => {
    setReviews((reviews) =>
      [...reviews].map((item) =>
  ...
  return (
    <div className="fixed bg-[#16191b3f] inset-0 z-10 flex justify-center items-center">
      ...
      ></div>
      <form
        className="relative z-30 w-1/2 bg-white rounded-lg p-7 h-3/4"
        onSubmit={async (e) => {
          e.preventDefault();
          [...reviews].forEach(async (item) => {
            ...
            await updateDoc(docRef, {
              rates: arrayUnion({
                ...
                star: item.stars,
                review: e.target.elements[item.product_id].value,
      ...
      >
        ...
        <div className="h-[85%] py-2 overflow-scroll scroll-hidden divide-y-2 divide-dashed divide-black">
          {reviews.map((review) => (
            <div className="py-5 space-y-2" key={uuidv4()}>
              <ProductReview review={review} onMouseEnter={onMouseEnter} />
              <textarea
        ...
        <div className="py-2">
          <Button
            type={"submit"}
            className="px-2 py-1 rounded-lg bg-[#4d7a94] text-white max-w-max ml-auto flex justify-end"
          >
            Hoàn thành
          </Button>
        </div>
...
const ProductReview = ({ review, onMouseEnter }) => {
  return (
    <>
      <div className="flex gap-5">
        <img
          ...
          alt=""
          className="w-16 h-20 rounded-lg shrink-0 grow-0"
        />
        <div className="flex flex-col py-2">
          <Link className="text-lg font-semibold line-clamp-2">
            {review.product_name}
          </Link>
          {Object.keys(review.product_properties).length > 0 && (
            <div className="flex items-center gap-2">
              <p>Phân loại: </p>
              {review.product_properties?.color && (
                <>
                  <SelectColorItem
                    size="0.8rem"
                    color={review.product_properties?.color.colorValue}
                    colorName={review.product_properties?.color.colorName}
                  ></SelectColorItem>
                  <p>{review.product_properties?.color.colorName}</p>
                </>
              )}
              {Object.keys(review.product_properties).map(
                (property) =>
                  property !== "color" && (
                    <p
                      key={uuidv4()}
                      className="px-1 border border-black rounded-lg"
                    >
                      {review.product_properties[property]}
                    </p>
                  )
              )}
            </div>
      ...
      <div className="flex items-center w-1/2 gap-1">
        {arrStar.map((item) =>
          item <= review.stars ? (
            <StarFull
              key={item}
              onMouseEnter={() => onMouseEnter(item, review.product_id)}
            />
          ) : (
            <Star
              key={item}
