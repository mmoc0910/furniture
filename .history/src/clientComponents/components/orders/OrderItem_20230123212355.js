...
const ModalReview = ({ products, onClick }) => {
  ...
  const onMouseEnterStarFull = (value, product_id) => {
    console.log(value);
    console.log(product_id);
  };
  const onMouseEnterStar = (value, product_id) => {
    console.log(value);
    console.log(product_id);
  };
  return (
        ...
        <div className="h-[85%] py-2 overflow-scroll scroll-hidden divide-y-2 divide-dashed divide-black">
          {reviews.map((review) => (
            <ProductReview
              key={uuidv4()}
              review={review}
              onMouseEnterStarFull={onMouseEnterStarFull}
              onMouseEnterStar={onMouseEnterStar}
            />
          ))}
        </div>
        <div className="py-2">
          <Button className="px-2 py-1 rounded-lg bg-[#4d7a94] text-white max-w-max ml-auto flex justify-end ">
            Hoàn thành
          </Button>
        </div>
      </div>
...
const ProductReview = ({ review, onMouseEnterStar, onMouseEnterStarFull }) => {
  return (
    <>
          ...
          {arrStar.map((item) =>
            item <= review.stars ? (
              <StarFull
                key={item}
                onMouseEnter={() =>
                  onMouseEnterStarFull(item, review.product_id)
                }
              />
            ) : (
              <Star
                key={item}
                onMouseEnter={() => onMouseEnterStar(item, review.product_id)}
              />
            )
          )}
        </div>
