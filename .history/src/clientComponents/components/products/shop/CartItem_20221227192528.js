  ...
  return (
    <>
      <tr className="border-b border-[#efefef] border-solid">
        <td className="px-3">
          {/* <div className="inline-block w-3 h-3 mx-auto bg-black"></div> */}
          <input
            type="checkbox"
            name="check"
            ...
            className="accent-[#e53637] md:block hidden peer"
            onChange={(e) => {
              if (e.target.checked) {
          ...
          <div className="flex flex-col gap-3 md:flex-row">
            ...
            <div className="shrink-0 grow-0 md:w-[calc(100%-6rem-0.75rem)]">
              <Link
                ...
                // to={(location) => ({
                //   ...location,
                //   pathname: `/shop/${cart.productInfo.slug}`,
                // })}
              >
                {cart.productInfo.productName}
              </Link>
              <div className="items-start">
                <p className="text-left text-[#b7b7b7] mb-1">Phân loại hàng:</p>
                <div className="flex items-center space-x-3">
                  {/* { Object.keys(cart.properties).map(prop => {
                    if()
                  })} */}
