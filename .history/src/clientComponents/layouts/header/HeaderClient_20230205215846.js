...
const Chat = () => {
  ...
  return (
    <div className="fixed z-50 w-16 h-16 right-10 bottom-10">
      <div
        className={`absolute bg-white shadow-chat rounded-lg bottom-8 right-20 transition-all duration-200 ${
          state ? "w-[330px] h-[450px]" : "w-0 h-0"
        }`}
      >
        <div
          className="flex flex-col justify-between w-full h-full px-5 overflow-hidden"
        ...
        >
          ...
          <div className="py-3">
            ...
            >
              <input
                className="px-3 py-1 bg-[#efefef] rounded-full outline-none max-w-[77%] w-full"
                ref={colRef}
              />
              <label htmlFor="image">
                <svg
...
const Message = ({ message }) => {
  ...
  return (
    <div onClick={() => setShow((show) => !show)}>
      <div
        className={`px-3 leading-tight py-1 rounded-lg w-max max-w-[75%] mt-1 break-words ${
          message.isAdmin
