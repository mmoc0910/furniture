...
const ShopPage = () => {
  ...
  const queries = (coll, startAfter) => {
    let q;
    if (!filter.categoryId && !filter.search && !filter.sortBy) {
      q = query(
        coll,
    ...
    if (filter.sortBy && !filter.categoryId && !filter.search) {
  ...
  const fetchMore = () => {
    setLoading(true);
    const colRef = collection(db, "products");
    const q = queries(colRef, startAfter(lastDoc));
    onSnapshot(q, (snapshot) => {
