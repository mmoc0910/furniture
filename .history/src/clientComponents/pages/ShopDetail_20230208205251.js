  ...
  return (
    <>
      <Breadcrumb pageName="" bgColor="white" color="black" bgImg={false}>
        ...
        <p className="text-[#b7b7b7] line-clamp-1 max-w-[350px]">
          {product.productName}
        </p>
      </Breadcrumb>
        ...
        <div className="col-span-1">
          ...
          <div className="pl-5 mt-6">
            ...
            <div className="flex flex-wrap items-stretch gap-5 mt-10">
              ...
              <Button
                className="px-4 py-2 font-bold text-white uppercase bg-black"
                onClick={async () => {
                  if (auth.currentUser) {
                    checkError();
                    if (Object.keys(err).length === 0) {
      ...
      <div className="container mb-24">
        <div className="flex items-center justify-center gap-x-20 font-bold text-[#b7b7b7] text-xl border-b border-solid border-[#e4e4e4]">
          <div
        ...
        <div className="relative mt-14">
          {state === 0 && (
            <DetailedOverview detailedOverview={product.detailedOverview} />
          )}
          {state === 1 && (
            <>
...
const DetailedOverview = ({ detailedOverview }) => {
  const [state, setState] = React.useState(0);
  const elRef = React.useRef(null);
  React.useEffect(() => {
    if (elRef.current) {
      if (elRef.current.scrollHeight <= 400) {
        setState(2);
      } else if (elRef.current.scrollHeight >= 400) {
        setState(0);
      }
    }
  }, [detailedOverview]);
  return (
    <>
      <div
        ref={elRef}
        className={`max-w-[90%] mx-auto overflow-hidden ${
          state ? "h-auto" : "max-h-[290px]"
        }`}
      >
        {parse(detailedOverview)}
      </div>
      <div>
        {state === 0 ? (
          <svg
...
const RelatedProducts = ({ priviewMode, categoryId, productId }) => {
              ...
              {products &&
                products.map((product) => (
                  <SwiperSlide key={uuidv4()}>
                    <ProductItem product={product} baseURL="../"></ProductItem>
                  </SwiperSlide>
                ))}
            </Swiper>
          </div>
