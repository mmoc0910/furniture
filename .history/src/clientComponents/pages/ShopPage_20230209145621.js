...
const ShopPage = () => {
  ...
  React.useEffect(() => {
    // const sortby = filter.sortBy ? orderBy("price", filter.sortBy) : orderBy("price", filter.sortBy);
    const search = where("productName", ">=", filter.search);
  ...
  const queries = (coll) => {
    const sortby = filter.sortBy ? orderBy("price", filter.sortBy) : null;
    const search = filter.search
      ? where("productName", ">=", filter.search)
      : null;
    const categoryId = filter.categoryId
      ? where("categoryId", "==", filter.categoryId)
      : null;
